...
function atcb_generate(button, buttonId, data) {
  ...
  buttonTrigger.innerHTML = '<span class="atcb_icon"><i data-feather="calendar" aria-hidden="true"></i></span>';
  ...
  data['options'].forEach(function(option) {
    ...
    switch (optionParts[0]) {
      ...
      case "Microsoft365":
        ...
        optionItem.innerHTML += '<span class="atcb_text">';
        optionItem.innerHTML += optionParts[1] || 'Microsoft';
        optionItem.innerHTML += '</span>';
        optionItem.addEventListener('click', function() {
...
function atcb_open() {
  ...
  document.getElementById('atcb_bgoverlay_' + this.dataset.atcbtn).classList.add('active');
}

function atcb_close() {
  // remove the active class, hide the list and the background overlay
  this.classList.remove('active');
  gsap.to(this, { zIndex: "80", duration: 0, delay: 0.4 });
  let list = document.getElementById('atcb_list_' + this.dataset.atcbtn);
  list.classList.remove('active');
  //document.getElementById('atcb_bgoverlay_' + this.dataset.atcbtn).style.display = 'none';
  document.getElementById('atcb_bgoverlay_' + this.dataset.atcbtn).classList.remove('active');
}
